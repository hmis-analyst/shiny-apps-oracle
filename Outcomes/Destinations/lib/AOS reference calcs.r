...
query <- read.csv(paste("Exits",prog_type,"SFY2014.csv",sep="_"))
...
MLE <- read.csv(paste("Z:/GitHub/shiny-apps-oracle/Outcomes/Destinations/lib/MLE",prog_type,"SFY2014.csv",sep="_"))
...
program_data <- data.frame(
  ...
  O_DESTTYPE_P = ifelse(is.na(query$DEST_PERMANENT),0,query$DEST_PERMANENT)
)
head(program_data)
...
program_data_agg4 <- read.csv("Prog data_ES_SFY2014.csv")
