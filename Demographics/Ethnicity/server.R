...
output$Plot <- renderPlot({
  ...
  input$update
  graphData <- data.frame(Ethnicity = c('Non-Hispanic','Hispanic'),Value=c(queryResults()[[1]],queryResults()[[2]]))
  progress <- Progress$new(session)
  ...
  isolate(  
    print(
      ...
      # Define variables
      ggplot(graphData,aes(x=factor(Ethnicity,levels=Ethnicity),y=Value)) + 
        # Define as bar chart
        geom_bar(fill=c("steelblue1", "seagreen2"),stat='identity') + 
        # Define title
        ggtitle(paste("Ethnicity Breakdown for ", ifelse(input$reportLevel!="Program","Programs in ",paste(input$agencySelect,": ",sep="")),
